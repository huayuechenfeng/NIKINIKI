# NIKINIKI 1.3.0 LGPL 重链接材料

GitHub Release `v1.3.0` 的 `NIKINIKI_1.3.0_relink_materials.zip` 让接收者可以取得构建 1.3.0
时静态链接的 FFmpeg 源码、完整 NIKINIKI 产品源码和未签名安装包，进行修改、重建和自行签名。
材料包不包含任何证书、私钥、账号资料或 SDK 构建目录。

## 内容

- `NIKINIKI_1.3.0_release_unsigned.sis`：与正式 SIS 对应的未签名包；大小和 SHA-256 见材料包内
  `SHA256SUMS.txt`；
- `NIKINIKI-source-v1.3.0.zip`：与 `v1.3.0` tag 相同的产品源码快照；
