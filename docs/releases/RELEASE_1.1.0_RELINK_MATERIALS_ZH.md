# NIKINIKI 1.1.0 LGPL 重链接材料

GitHub Release `v1.1.0` 的 `NIKINIKI_1.1.0_relink_materials.zip` 让接收者可以取得
构建 1.1.0 时静态链接的 FFmpeg 源码、完整 NIKINIKI 产品源码和未签名安装包，进行
修改、重建和自行签名。材料包不包含任何证书、私钥、账号资料或 SDK 构建目录。

## 内容

- `NIKINIKI_1.1.0_release_unsigned.sis`：与正式 SIS 对应的未签名包；SHA-256 为
  `F6B776AA47C2129B161128ABCD473856F8CD03E61752E17D64BEB72EAC3790E7`；
- `NIKINIKI-source-v1.1.0.zip`：与 `v1.1.0` tag 相同的产品源码快照；
