...
1.0.0 首次运行可能在应用内同步读取完整字体资源；这不是向 Symbian 系统安装字体。保持
应用前台或黑屏状态约 60 秒，然后从后台关闭并重新启动。若仍然黑屏，重启手机后再试。
成功启动一次后通常不会重复发生。当前主线已有非阻塞修复，但在取得真机冷启动结果前，
本节仍按已发布的 1.0.0 行为提供处理方法。
...
这是 1.0 的已知限制。切换 CDN 或反复请求更低清晰度不一定改变 H.264 结构。
...
公开用户只应安装正式 Release SIS。Debug、unsigned、`surfacepersist1`、`codeccompat1`、
