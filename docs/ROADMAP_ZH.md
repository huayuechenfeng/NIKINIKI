...
## Now：验收系统播放器的 360P MP4 交接，并维持 1.2 基线

### 0. 外部播放器交接

- 播放方式提供内置/系统播放器各三项选择；系统“流式”直接交接已核对的 360P MP4
  HTTP(S) 签名地址，系统“边下边播”和“下载后播放”均先完整下载并核验本地 MP4；
- 远程链接交接不能附带 NIKINIKI 的 Cookie/Referer；本地交接不透传签名 URL、Cookie、
  Referer 或增长中文件；
- 交接前停止并停放内部媒体、解码器、计时器与下载回调，恢复稳定竖屏和前台所有权，
  再以 Symbian 文档交接打开带 `video/mp4` 类型的链接或完整本地文件；
...
当前源码已实现上述链接直交、完整下载、AppArc 交接、旧设置迁移和返回状态重置。
普通 Debug/Release GCCE 构建、SIS 打包、文档、公开边界及主机检查已通过；本阶段剩余门槛
是独立真机验收，不把 API 接受、主机检查或编译通过升级为设备播放通过。
