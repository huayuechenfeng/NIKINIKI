...
## Now：维护 1.3 系统播放器兼容基线

1.3 已完成 360P MP4 的系统播放器链接/本地文件交接并取得用户功能验收。当前不再扩展新的
外部播放后端；后续只按[设备测试矩阵](reference/DEVICE_TEST_MATRIX.md)补充不同固件、关联处理器、
HTTPS、返回与重复使用结果。产品行为和准确边界见[当前状态](STATUS_ZH.md)与
[1.3.0 发布说明](releases/RELEASE_1.3.0_ZH.md)。

### 1. 播放器稳定性

- 在 1.3 正式 Release 上完成 50 次进入、播放、退出和再次进入；
...
- 在原版 Symbian³、Anna 上安装与 Belle 相同的 1.3.0 正式 SIS；
